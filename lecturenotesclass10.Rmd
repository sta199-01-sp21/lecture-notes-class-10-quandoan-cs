...
- How many patients were enrolled in the clinical trial?
  ...
  84 patients, through the sum.
  
- What is the probability a randomly selected patient received the placebo?

  43 sum of people who receive placebo/84 total people. (Marginal Probability.)
  51%
  
- What is the probability a randomly selected patient received the placebo
  and had a marked improvement?
  
  7 marked for placebo/ 84 total people. (8.3%)
  
- What is the probability a randomly selected patient received the placebo
  and the treatment?
  
  Not possible, disjoint probability because not possible. 
  
  
- What is the probability a randomly selected patient had some improvement or
  was on the treatment?
  
  41/84 because it doesn't have to be both of them.
...
c command to combine things together into a vector.
...
Very close, law of large numbers allow the numbers to be even closer to the 
true proportion if we increase our sample size.
...
```{r simulate_rolls}
roll_results <- tibble(
  dice_1 = replicate(n = 10000, expr = sample(1:6, size = 1)),
  dice_2 = replicate(n = 10000, expr = sample(1:6, size = 1))
)
roll_results
```
...
roll_results <- roll_results %>%
  mutate(event_1 = dice_1 ^ 2 == dice_2,
         event_2 = abs(dice_1 - dice_2) == 4)
roll_results
```
